...
TOKEN=$(curl -s -X PUT "http://localhost:1338/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600" || echo "error")
...
while sleep 5; do
    ...
    HTTP_CODE=$(curl -H "X-aws-ec2-metadata-token: $TOKEN" -s -w "%{http_code}" -o /dev/null "http://localhost:1338/latest/meta-data/spot/instance-action")
