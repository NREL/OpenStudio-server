...
fmin <- function(x) - f(x)
results <- NULL
try(results <- gaisl(type = 'real-valued', fitness=fmin, min=varMin, max=varMax, popSize=popSize, pcrossover=pcrossover, pmutation=pmutation, elitism=elitism, maxiter=maxiter, run=run, maxFitness=(-1*maxFitness), names=varnames, monitor=TRUE, parallel=length(ips)), silent=FALSE)
