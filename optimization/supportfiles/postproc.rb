...
def sql_query2(sql, query)
  val = sql.execAndReturnFirstDouble("SELECT Value FROM TabularDataWithStrings WHERE ReportName='Life-Cycle Cost Report' AND #{query}").get
  return val
end

def add_element(hash, var_name, value, xpath = nil)
  values_hash = OrderedHash.new()
...
begin
  ...
  tbl_data << add_data2(sql_file, "TableName='Present Value by Category' AND RowName='Grand Total' AND ColumnName='Present Value'", "Cost", nil, nil)
      ...
      if rowcnt == 1
        ...
        row.each do |col|
          ...
          case col
            ...
            when "Cost"
              results["#{colcnt}"] = {:name => "cost", :units => units, :longname => longname}  
