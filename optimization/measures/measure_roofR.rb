#start the measure
class SetInsulationRValueForRoofs < OpenStudio::Ruleset::ModelUserScript
  ...
  def run(model, runner, user_arguments)
    ...
    #unit conversion of roof insulation from IP units (ft^2*h*R/Btu) to SI units (M^2*K/W)
    ...
    if exterior_surfaces.empty?
      runner.registerAsNotApplicable("The model does not have any roofs, nothing will be changed.")
    end
    ...
    exterior_surface_constructions.each do |exterior_surface_construction|
      ...
      if not thermal_resistance_values.max > min_expected_r_value_si.value
        ...
        final_construction.setName("#{exterior_surface_construction.name.to_s}_adj_roof_insulation")
        ...
        if found_material == false
          ...
          new_material.setName("#{max_thermal_resistance_material.name.to_s} R-value #{r_value} ft^2*h*R/Btu")
          materials_hash[max_thermal_resistance_material.name.to_s] = new_material
          final_construction.eraseLayer(max_thermal_resistance_material_index)
          final_construction.insertLayer(max_thermal_resistance_material_index,new_material)
    ...
    default_construction_sets.each do |default_construction_set|
      if default_construction_set.directUseCount > 0
        ...
        if not default_surface_const_set.empty?
          ...
          new_default_construction_set.setName("#{default_construction_set.name.to_s}_adj_roof_insulation")
          ...
          new_default_surface_const_set.setName("#{default_surface_const_set.get.name.to_s}_adj_roof_insulation")
          ...
          if not target_const.empty?
            ...
            if found_const_flag == false # this should never happen but is just an extra test in case something goes wrong with the measure code
              runner.registerWarning("Measure couldn't find the construction named '#{target_const}' in exterior surface hash.")
            end
          end
