  ...
  def run(model, runner, user_arguments)
    ...
    thermostats.each do |thermostat|
      ...
      if not clg_set_sch.empty?
        ...
        if not clg_set_schs[clg_set_sch.get.name.to_s] == nil
      ...
      if not htg_set_sch.empty?
        ...
        if not htg_set_schs[htg_set_sch.get.name.to_s] == nil
    ...
    clg_set_schs.each do |k,v| #old name and new object for schedule
      if not v.to_ScheduleRuleset.empty?
        rules = v.to_ScheduleRuleset.get.scheduleRules
        rules.each do |sch_rule|
