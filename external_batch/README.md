...
- UrbanOpt analyses and per-analysis `gemfile` are not supported (packager
  raises) — both need a bundle install the executor environment doesn't have.
...
- Preflight histogram images are not generated by the Ruby sampling backend
  (they need R/cairo; the Rserve backend still produces them).
- Transport to remote executors is manual/scripted (shared dir, rsync, or
  s3 sync); no built-in S3 credential handling yet. A shared-filesystem
  executor (e.g. a Nomad job running the `local_executor.rb` pattern against
  mounted storage) needs no transport step at all.
