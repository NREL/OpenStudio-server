...
if [ -e ${GEMFILE_DIR}/Gemfile.lock ]; then
  ...
  if [ -z $LOCAL_BUNDLER_VERSION ]; then
   ...
   gem install --install-dir /var/oscli/gems/ruby/2.7.0 bundler -v $LOCAL_BUNDLER_VERSION
...
bundle '_'$LOCAL_BUNDLER_VERSION'_' install --path gems --retry 10 --full-index
