...
class AnalysesController < ApplicationController
  ...
  def batch_create
    ...
    # 1. Save the uploaded file to a persistent location in the shared assets directory
    bulk_uploads_dir = File.join(APP_CONFIG['server_asset_path'], 'bulk_uploads')
    FileUtils.mkdir_p(bulk_uploads_dir)
    persistent_zip_path = File.join(bulk_uploads_dir, "bulk_#{SecureRandom.uuid}.zip")
    File.open(persistent_zip_path, 'wb') { |f| f.write(uploaded_file.read) }

    # 2. Queue the background job
    if Rails.application.config.x.job_manager == :delayed_job
      Delayed::Job.enqueue DjJobs::ProcessBulkUpload.new(persistent_zip_path, @project.id.to_s), queue: 'analyses'
    elsif Rails.application.config.x.job_manager == :resque
      Resque.enqueue(ResqueJobs::ProcessBulkUpload, persistent_zip_path, @project.id.to_s)
    else
      raise 'Rails.application.config.x.job_manager must be set to :resque or :delayed_job'
    end

    render json: { status: 'success', created: [] }, status: :created
  end
